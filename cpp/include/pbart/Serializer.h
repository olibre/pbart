...
namespace pbart
{
class PBART_EXPORTS Serializer
{
public:
    // Initialize Dico -----------------------------------------------------
    ...
    void     encode (const Message&, CHAR_T preallocated[], SIZE_T size=0) const; //, POS_T pos=0) const;
...
private:
    ...
    // static data used by header()
    static const std::string nodico_;
...
inline void swap (pbart::Dico& d1, pbart::Dico& d2)
{
    d1.swap(d2);
}
...
inline Message Serializer::decode (const Buffer& buffer, Id requestedId) const
{
    ...
    if (! buffer.empty())
    {
        ...
        size_t size = Codec::bufferToPtr (buffer, ptr);
        assert( size <= INT_MAX );
        Codec::decode (msg, ptr, static_cast<int>(size), false);
    }
...
inline int Serializer::decode (const Buffer& buffer, Message& msg, bool concatenated, bool dico)
{
    ...
    size_t size = Codec::bufferToPtr (buffer, ptr);
    assert(size <= INT_MAX);
    return decode (ptr, static_cast<int>(size), msg, concatenated, dico);
}
...
//--------------------------------------------------------------------------
template <typename CHAR_T, typename SIZE_T> //, typename POS_T>
inline void Serializer::encode (const Message& msg, CHAR_T preallocated[], SIZE_T size) const //, POS_T pos) const
{
    if (msg.id())
    {
