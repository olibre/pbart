...
//Id Serializer::dicoId_;
  ...
  //--------------------------------------------------------------------------
  void Serializer::loadDico (const std::string& file)   //, bool reload)
  {
    Dico* ptr;
    ...
    // if (reload)
    // {
    //   // TODO(Oliver):check changes between pDico and dico (importance of changes)
    //   // if the current pDico_ must not be changed by the new loaded one
    //   // TODO(Oliver):And what about the Messages using the previous dico ???
    // }
  ...
  void Serializer::addItem (const Variant::SeqMessage::const_iterator jt, Field& field)
  {
    ...
    signed signedId = jt->get(idId_).toLong();
    Type   type     = jt->get(typeId_).toLong();
