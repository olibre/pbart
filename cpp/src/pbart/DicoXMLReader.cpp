...
void DicoXMLReader::parseFieldDefElt (tinyxml2::XMLElement *elem)
{
    ...
    {
        std::ostringstream oss;
        oss <<"Field '"<< name <<"' cannot have id="<< signedId <<" greater than "<< ID_MAX << context(*elem);
        const std::string& str = oss.str();
        throw DicoException (str);
    }
    Id  id = signedId;
    ...
    {
    default:
        assert(0&&"Expect only field.type() MESSAGE and EMPTY. Another unvalid type.");
...
void DicoXMLReader::parseMessageDefElt (tinyxml2::XMLElement *elem)
{
    ...
    {
        std::ostringstream oss;
        oss <<"Message '"<< name <<"' cannot have id="<< signedId <<" greater than "<< ID_MAX << context(*elem);
        const std::string& str = oss.str();
        throw DicoException (str);
    }
    Id id = signedId;
         ...
         child; child = child->NextSiblingElement())
    {
        ...
        if (MSG_FIELD_ELT == childname || MSG_MESSAGE_ELT == childname)
        {
...
const char* DicoXMLReader::getStringAttribute (tinyxml2::XMLElement *elem, const char name[], const bool required)
{
    ...
    if (required)
    {
        std::string txt = "Missing attribute '";
        txt += name;
        txt += "' (the value can be any string)";
        txt += context(*elem);
        throw DicoException (txt);
    }
    else
    {
        return "";
