...
//not used
//
//  std::string ExceptionHelper::dico (const std::string& file, const std::string& reason)
//  {
//    std::string str;
//    str += "Dictionnary file=";
//    str += file;
//    str += " not loaded because ";
//    str += reason;
//    return str;
//  }
  ...
  static inline std::string decodeError (Id id, const std::string& dicoHeader, const std::string& msgHeader)
  {
    ...
    for (std::string::const_iterator it = dicoHeader.begin(); it != dicoHeader.end(); oss <<'['<< (int) *it++ <<']');  //NOLINT
