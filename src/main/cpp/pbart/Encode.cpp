...
void Codec::encode (const Message& message, Buffer& buffer)
{
    ...
    gpi::WireFormatLite::WriteBytes (static_cast<int>(message.id()), message.dico()->header(), &output);
...
inline void Codec::encodeString (Id id, const std::string& str, gp::io::CodedOutputStream& output)
{
    ...
    gpi::WireFormatLite::WriteString (static_cast<int>(id), str, &output); //UTF-8 specific
#else
    gpi::WireFormatLite::WriteBytes  (static_cast<int>(id), str, &output); // WriteBytes() equals WriteString() [v2.4.1]
#endif
}
...
#else
    gpi::WireFormatLite::WriteTag (static_cast<int>(id), gpi::WireFormatLite::WIRETYPE_LENGTH_DELIMITED, &output);
...
inline void Codec::encodeSeqBool (Id id, const Variant::SeqBool& v, gp::io::CodedOutputStream& output)
{
...
#endif
    {
        ...
        output.WriteVarint32 (static_cast<uint32_t>(v.size()));
...
inline int Codec::lengthSeqLong (const Variant::SeqLong& v)
{
    ...
    for (size_t i = 0; i < v.size(); ++i) {
        assert( length <= INT_MAX - gpi::WireFormatLite::Int64Size(v[i]) );
...
{
    assert(    UINT32_MAX >= 8 * v.size() );
    return static_cast<int>( 8 * v.size() ); //buffer size required to store the whole array
}
...
inline int Codec::lengthSeqString (const Variant::SeqString& v, int tagSize)
{
    ...
    int length = 0; //buffer size required to store the whole array
#else
    assert( tagSize * v.size() <= INT_MAX );
    int length = tagSize * static_cast<int>(v.size());
#endif
    for (size_t i = 0; i < v.size(); ++i) {
        assert( length <= INT_MAX - gpi::WireFormatLite::BytesSize (v[i]) );
        length += gpi::WireFormatLite::BytesSize (v[i]); //BytesSize() equals StringSize()
    }
    return length;
}
...
inline void Codec::encodeSeqString (Id id, const Variant::SeqString& v, gp::io::CodedOutputStream& output)
{
    ...
    {
        gpi::WireFormatLite::WriteTag (static_cast<int>(id), gpi::WireFormatLite::WIRETYPE_LENGTH_DELIMITED, &output);
        assert( v[i].size() <= UINT32_MAX );
        output.WriteVarint32 (static_cast<int>(v[i].size()));
        output.WriteString (v[i]);  //in v2.4.1 -> calls WriteRaw()
    }
#endif
}
...
inline int Codec::lengthSeqMessage (const Variant::SeqMessage& v, int tagSize)
{
    ...
    {
#ifdef USE_INT32_TO_SERIALISE_SUB_MESSAGE
        assert( length <= INT_MAX - gpi::WireFormatLite::Int32Size (v[i].size()) );
        length += gpi::WireFormatLite::Int32Size (v[i].size()); //number of fields within the message
        assert( length <= INT_MAX - Codec::length (v[i]) );
        length += Codec::length (v[i]);
#else
        int32_t bytes = Codec::length(v[i]);
        assert( length <= INT_MAX - gpi::WireFormatLite::Int32Size (bytes) - bytes );
...
int Codec::calculateLength (Message const& msg)
{
         ...
         ++it)
    {
        ...
        case Type::BOOL:
            total += gpi::WireFormatLite::TagSize (static_cast<int>(it->first), gpi::WireFormatLite::TYPE_BOOL);
            total += 1;
            break;

        case Type::LONG:
            total += gpi::WireFormatLite::TagSize (static_cast<int>(it->first), gpi::WireFormatLite::TYPE_INT32);
            total += gpi::WireFormatLite::Int64Size (it->second.toLong(std::nothrow));
            break;

        case Type::DOUBLE:
            total += gpi::WireFormatLite::TagSize (static_cast<int>(it->first), gpi::WireFormatLite::TYPE_DOUBLE);
            total += 8;
            break;

        case Type::STRING:
            total += gpi::WireFormatLite::TagSize (static_cast<int>(it->first), gpi::WireFormatLite::TYPE_STRING);
...
void Codec::encode (Message const& msg, gp::io::CodedOutputStream& output)
{
         ...
         ++it)
    {
        ...
        {
        case Type::EMPTY:
            assert(0&&"Unexpected Type::BOOL from variant.type()");
            BOOST_FALLTHROUGH; // for optimization continue with next case
        default:
