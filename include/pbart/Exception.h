...
//  then the class must be declare using PBART_EXPORTS
//  all all the classes it derives must also be PBART_EXPORTS
//  As pbart::Exception derives from std::logic_error
//  and this class is not declared using PBART_EXPORTS
//  therefore pbart::Exception ans its derivated classes cannot
//  define body in *.cpp  :(
//--------------------------------------------------------------------------
class PBART_EXPORTS ExceptionHelper
{
public:
...
// TODO(olibre): Prevent vtable be in all translation units
//               for below base class and all derivated classes
